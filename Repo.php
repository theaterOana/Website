...
class Repo
{
    ...
    public function isActive($UserName)
    {
        ...
        try {
            ...
            // set the PDO error mode to exception
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT Active FROM `Leden` WHERE UserName LIKE :UserName");
    ...
    public function getPasswordForUser($username)
    {
        try {
            ...
            // set the PDO error mode to exception
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT Wachtwoord FROM `Leden` WHERE UserName LIKE :UserName");
            $stmt->bindValue(":UserName", $username);
            $stmt->execute();
            $result = $stmt->fetchAll();
//            print_r( $result[0]["Wachtwoord"]);
    ...
    public function Register($Voornaam, $Famielienaam, $Wachtwoord, $UserName)
    {
        try {
            ...
            // set the PDO error mode to exception
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("INSERT INTO `Leden`(`Voornaam`, `Familienaam`, `Wachtwoord`, `Active`, `UserName`) VALUES (:Voornaam,:Familienaam,:Wachtwoord,false ,:Username)");
